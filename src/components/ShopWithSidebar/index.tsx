...
import ProductListSkeleton from "../Ui/Skeleton/ProductListSkeleton";
...
const ShopWithSidebar = ({ 
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      url += `min_price=${priceRange.from}&max_price=${priceRange.to}&`;
      ...
      url += `sort=${sortMap[sortBy] || "created_at_desc"}`;
  ...
  useEffect(() => {
    // Only fetch on client if it's not the initial mount or if initialProducts was empty
    if (initialProducts.length === 0 || products !== initialProducts || selectedCategories.length > 0 || sortBy !== "0" || currentPage !== 1) {
       fetchProducts();
    }
      ...
      <section className="overflow-hidden relative pb-20 pt-5 lg:pt-20 xl:pt-28 bg-gray-1">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            ...
            {productSidebar && (
              <div
                className="fixed inset-0 bg-dark/50 z-999 xl:hidden"
            ...
            {!productSidebar && (
              <button
                ...
                className="xl:hidden fixed left-0 top-1/2 -translate-y-1/2 z-999 flex flex-col items-center justify-center gap-2 bg-blue text-white py-3 px-1 rounded-r-md shadow-lg"
              >
                <span className="[writing-mode:vertical-lr] rotate-180 font-medium uppercase tracking-widest text-xs">
                  Filter
                </span>
                <svg
                  width="16"
            ...
            >
              ...
              <div className="xl:hidden flex justify-end mb-4">
                <button
                  onClick={() => setProductSidebar(false)}
                  className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-1 text-dark hover:bg-gray-2"
                >
                  <svg
                    width="20"
                    height="20"
            ...
            <div className="xl:max-w-[870px] w-full">
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <CustomSelect options={options} onChange={(opt) => setSortBy(opt.value)} />

                    <p>
                      Showing <span className="text-dark">{products.length} of {meta?.total || 0}</span>{" "}
                      Products
                    </p>
                  </div>
              ...
              <div
                className={`${
                  productStyle === "grid"
                    ? "grid grid-cols-2 lg:grid-cols-3 gap-x-4 sm:gap-x-7.5 gap-y-6 sm:gap-y-9"
                ...
                <div className="flex justify-center mt-15">
                  <div className="bg-white shadow-1 rounded-md p-2">
                    <ul className="flex items-center">
                      ...
                      {meta.links.filter((l: any) => !isNaN(Number(l.label))).map((link: any, key: number) => (
                        <li key={key}>
                          <button
                            onClick={() => link.url && setCurrentPage(Number(link.label))}
