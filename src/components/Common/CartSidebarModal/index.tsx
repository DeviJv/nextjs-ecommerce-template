...
const CartSidebarModal = () => {
  ...
  const handleCheckout = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/checkout/paypal`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-99999 w-full h-screen bg-dark/70 transition-transform duration-300 ${isCartModalOpen ? "translate-x-0" : "translate-x-full"
        }`}
    >
      <div className="flex justify-end h-full">
        <div className="w-full max-w-[500px] h-full shadow-1 bg-white px-4 sm:px-7.5 lg:px-11 relative modal-content flex flex-col uppercase">
          ...
          {cartItems.length > 0 && (
            <div className="border-t border-gray-3 bg-white pt-5 pb-4 sm:pb-7.5 lg:pb-11 mt-7.5 sticky bottom-0">
              <div className="flex items-center justify-between gap-5 mb-6">
                <p className="font-medium text-xl text-dark uppercase">Subtotal:</p>
                <p className="font-medium text-xl text-dark uppercase">${totalPrice}</p>
              </div>

              <div className="flex items-center gap-4">
                <Link
                  href="/checkout"
                  className="w-full flex justify-center font-medium text-white bg-dark py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-opacity-95 uppercase"
                >
                  Checkout
                </Link>
              </div>
